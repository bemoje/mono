...
Get the files that do not have full code coverage: run `yarn devkit missing-coverage-files`
...
Finally, when processing of all files is complete, run the tests with `yarn test-coverage` and then verify that all files have full code coverage by running `yarn devkit missing-coverage-files` again and ensuring that no files are printed.
