...
export class Help implements IHelp {
  ...
  visibleOptions(cmd: CommandHelp): OptionHelp[] {
    const visibleOptions = cmd.options.filter((option: OptionHelp) => !option.hidden && !option.global)
  ...
  visibleGlobalOptions(cmd: CommandHelp): OptionHelp[] {
    ...
    const globalOptions: OptionHelp[] = cmd.options.filter((option) => !option.hidden && !!option.global)
    for (let ancestorCmd = cmd.parent; ancestorCmd; ancestorCmd = ancestorCmd.parent) {
      const visibleOptions = ancestorCmd.options.filter((option: OptionHelp) => !option.hidden && !!option.global)
...
//  */
// export function _findCommand(this: Command, name: string) {
...
//  * @return {Option}
//  */
// export function _findOption(this: Command, arg: string): Option | undefined {
//   return this.options.find((option) => is(option, arg))
// }
...
//  * @package
//  */
// function _concatValue(ins: Option, value: string, previous: string) {
//   if (previous === ins.defaultValue || !Array.isArray(previous)) {
//     return [value]
//   }
...
// function is(ins: Option, arg: string) {
//   return ins.short === arg || ins.long === arg
// }
