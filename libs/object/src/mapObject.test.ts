...
describe(mapObjectKeys.name, () => {
  it('should map over the object keys and apply the transformation', () => {
    const input = { a: 1, b: 2 }
    const result = mapObjectKeys(input, (key, value) => {
      return `${key}${value}`
    })
  ...
  it('should handle empty objects gracefully', () => {
    const result = mapObjectKeys({}, (key) => {
      return key
    })
    expect(result).toEqual({})
  })
...
describe(mapObjectEntries.name, () => {
  it('should map over the entries of an object and return transformed entries', () => {
    const input = { a: 1, b: 2 }
    const result = mapObjectEntries(input, (key, value) => {
      return [`${key}_mapped`, value * 10]
    })
    expect(result).toEqual({ a_mapped: 10, b_mapped: 20 })
  })

  it('should handle empty objects gracefully', () => {
    const result = mapObjectEntries({}, (key, value) => {
      return [key, value]
    })
    expect(result).toEqual({})
  })
