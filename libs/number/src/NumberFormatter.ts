...
/**
 * A utility class for formatting and parsing numbers with locale-specific separators.
 *
 * This class allows for customizing thousand and decimal separators, setting precision
 * for decimal places, and supports different locales for international number formatting.
 *
 * @example
 * ```typescript
 * // Create a formatter with 2 decimal places
 * const formatter = new NumberFormatter(2);
 *
 * // Format a number
 * formatter.format(1234.56); // "1,234.56"
 *
 * // Parse a formatted string
 * formatter.parse("1,234.56"); // 1234.56
 *
 * // Use a different locale
 * formatter.locale("fr").format(1234.56); // "1 234,56" (assuming French locale is defined)
 * ```
 *
 * @remarks
 * The class maintains a static registry of locales with their respective thousand and decimal separators.
 * Before using a locale, it must be defined using the `defineLocale` static method.
 */
