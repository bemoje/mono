...
const removeDependency = new Template({
  strategy: new StringTemplateStrategy(),
  optionsSchema: Type.Object({ workspace: Type.String(), dependency: Type.String() }),
  template: 'yarn workspace {{workspace}} remove {{dependency}}',
})

const openFileInIDE = new Template({
  strategy: new StringTemplateStrategy(),
  optionsSchema: Type.Object({ filepath: Type.String() }),
  template: 'code {{filepath}}',
})

export const commands = { addDependency, addDevDependency, removeDependency, openFileInIDE }
